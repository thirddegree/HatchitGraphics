...
namespace Hatchit {
    ...
    namespace Graphics {
        ...
        namespace DirectX {
            ...
            D3D11Shader::D3D11Shader(ID3D11Device* device, ID3D11DeviceContext* context)
            {
                m_device = device;
                m_context = context;
            ...
            bool D3D11Shader::VInitFromFile(Core::File* file)
            {
            ...
            ConstantBufferVariable* D3D11Shader::FindVariable(std::string name, size_t size)
            {
            ...
            ConstantBuffer* D3D11Shader::FindBuffer(std::string name)
            {
                for (uint32_t i = 0; i < m_constantBufferCount; i++)
                {
            ...
            int32_t D3D11Shader::FindTextureBindIndex(std::string name)
            {
            ...
            int32_t D3D11Shader::FindSampleBindIndex(std::string name)
            {
            ...
            void D3D11Shader::DeActivate()
            {
                VUnbind();
            }

            void D3D11Shader::UpdateAllBuffers()
            {
                for (uint32_t i = 0; i < m_constantBufferCount; i++)
                {
            ...
            bool D3D11Shader::VSetData(std::string name, const void* data, size_t size)
            {
            ...
            bool D3D11Shader::VSetInt(std::string name, int data)
            {
                return VSetData(name, static_cast<void*>(&data), sizeof(int));
            }

            bool D3D11Shader::VSetFloat(std::string name, float data)
            {
                return VSetData(name, static_cast<void*>(&data), sizeof(float));
            }

            bool D3D11Shader::VSetFloat2(std::string name, Math::Vector2 data)
            {
                return VSetData(name, static_cast<void*>(data.getAsArray()), sizeof(float) * 2);
            }

            bool D3D11Shader::VSetFloat3(std::string name, Math::Vector3 data)
            {
                return VSetData(name, static_cast<void*>(data.getAsArray()), sizeof(float) * 3);
            }

            bool D3D11Shader::VSetFloat4(std::string name, Math::Vector4 data)
            {
                return VSetData(name, static_cast<void*>(data.getAsArray()), sizeof(float) * 4);
            }

            bool D3D11Shader::VSetMatrix3(std::string name, Math::Matrix3 data)
            {
                return VSetData(name, static_cast<void*>(data.getAsArray()), sizeof(float) * 12);
            }

            bool D3D11Shader::VSetMatrix4(std::string name, Math::Matrix4 data)
            {
